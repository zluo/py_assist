...
class Generator(object):
    ...
    def word_exchange(self, data, style):
        ...
        exchange = ecdict.stardict.tools.exchange_loads(exchange)
        if not exchange:
            return ''
        part = []
        last = ''
                ...
                if '1' in exchange:
                    ...
                    if 'p' in t and 'd' in t:
                        ...
                        for x in ('i', 'p', 'd', '3', 's', 'r', 't'):
                            if x in t:
                                derive = ecdict.stardict.tools._exchanges[x]
                                break
                    if derive:
    ...
    def word_pos(self, data):
        pos = ecdict.stardict.tools.pos_extract(data)
        if not pos:
            return None
        if len(pos) < 2:
            return None
    ...
    def compile_stardict(self, dictionary, filename, title):
        print('generating ...')
        words = ecdict.stardict.tools.dump_map(dictionary, False)
        out = {}
        pc = ecdict.stardict.tools.progress(len(words))
        for word in words:
            pc.next()
        ...
        pc.done()
        print('saving ...')
        ecdict.stardict.tools.export_stardict(out, filename, title)
        return pc.count
    ...
    def compile_mdx(self, dictionary, filename, mode=None, style=False):
        ...
        if mode is None:
            mode = ('name', 'phonetic')
        count = 0
        stripword = ecdict.stardict.stripword
    ...
    def compile_css(self, dictionary, filename, css=None):
        fp = codecs.open(filename, 'w', 'utf-8')
        text2html = self.text2html
        pc = ecdict.stardict.tools.progress(len(dictionary))
        if not css:
...
class Resemble(object):
    ...
    def load(self, filename):
        self._resembles = []
        self._words = {}
        file_content = ecdict.stardict.tools.load_text(filename)
    ...
    def compile_map(self):
        ...
        if (not self._resembles) or (not self._words):
            return False
        pc = ecdict.stardict.tools.progress(len(self._words))
    ...
    def compile_mdx(self, filename):
        ...
        desc += u'日期：%s</br>\n' % text
        desc += '</font>'
        ecdict.stardict.tools.export_mdx(words, filename, title, desc)
        return True
...
class Treasure(object):
    ...
    def get_exchange(self, data):
        ...
        if not exchange:
            return None
        chg = ecdict.stardict.tools.exchange_loads(exchange)
        if not chg:
            return None
        part = []
        last = ''
    ...
    def compile_mdx(self, db, name1, name2):
        mdx1 = {}
        mdx2 = {}
        pc = ecdict.stardict.tools.progress(len(db))
        for _, word in db:
            pc.next()
            data = db[word]
        ...
        if os.path.splitext(name1)[-1].lower() == '.mdx':
            ecdict.stardict.tools.export_mdx(mdx1, name1, 'anki-front')
        else:
            ecdict.stardict.tools.export_mdict(mdx1, name1)
        if os.path.splitext(name2)[-1].lower() == '.mdx':
            ecdict.stardict.tools.export_mdx(mdx2, name2, 'anki-back')
        else:
            ecdict.stardict.tools.export_mdict(mdx2, name2)
        return 0
...
if __name__ == '__main__':
    ...
    def test1():
        db = ecdict.stardict.open_local('stardict.db')
        data = db['higher']
    ...
    def test4():
        db = ecdict.stardict.open_local('treasure.db')
        data = db['breakup']
    ...
    def test6():
        db = ecdict.stardict.open_local('ultimate.db')
        data = db['sting']
        data['translation'] += u'\n> hahahah\n[网络] 你好'
        import StringIO
